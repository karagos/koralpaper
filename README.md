# KoralPaper — *Draw your thinking* (v3.64.0)
...
- **Design with Claude (MCP):** install the one-click **koralpaper.mcpb**
  ...
  the app
- **Brand kit (Settings):** your identity in one small file: name, six accent
...
- **Time-lapse recorder (☰):** press F to snapshot the page as a frame, or
  ...
  has a hold time and a move time: during the move, elements travel to the
  next frame in shaky stop-motion steps, strokes redrawn on every step like
  a hand-shot animation (the default), or glide smoothly with Smooth, Snappy,
  Overshoot or Linear easing. Appearing elements hard-cut, or fade if you
  tick Fade. Camera keyframes zoom and pan between frames,
